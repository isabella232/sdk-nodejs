...
*/
MP.prototype.getRefreshTokenByAuthCode = function (authCode, redirectUri, next) {
	MP.restClient.post(
		"/oauth/token",
		{
		...
		function(error, resp) {
			if (error) {
				...
				next (null, resp.response.refresh_token);
			}
		}
	);
};
		...
		MP.restClient.post(
			...
			function(error, resp) {
				if (error) {
					...
					__self.__marketplace.refreshToken = newRefreshToken;
					__self.__marketplace.refreshFunction(newRefreshToken);
...
MP.prototype.searchPayment = function (filters) {
	...
	this.getAccessToken (function (err, accessToken){
		...
		MP.restClient.get(
			"/collections/search?"+__self.__build_query(filters),
			config.MIME_JSON,
			next
		);
	});
...
MP.prototype.getAccountInfo = function (next) {
	if (!next) {
		throw new Error ("No callback function defined");
	}
