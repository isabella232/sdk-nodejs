...
var MP = function() {
  ...
  /**
	...
	*/
  mp.enableMarketplace = function(initRefreshToken, refreshFunction) {
    if (typeof refreshFunction != "function") {
      throw new Error(
        "Must provide a callback (to get the new refresh_token each refresh!)"
      );
    }
  ...
  /**
	Get first refresh_token based on authorization_code (after seller authorization)
	@param authCode authorization code retrieved by MercadoPago
	@param redirectUri uri where MercadoPago redirect after authorization
	@param next retrieves the first refresh token
	*/
  mp.getRefreshTokenByAuthCode = function(authCode, redirectUri) {
    var next =
    ...
    MPRestClient.post({
      ...
      function success(data) {
        if (data.status != 200) {
          next && next(data.response);
          deferred.reject(data.response);
        } else {
          next && next(null, data.response.refresh_token);
          deferred.resolve(data.response.refresh_token);
        }
      },
      function error(err) {
        next && next(err);
  ...
  mp.getAccountInfo = function(next) {
    var next =
      typeof arguments[arguments.length - 1] == "function"
        ? arguments[arguments.length - 1]
        : null;
      ...
      if (__marketplace) {
        ...
        MPRestClient.post({
          ...
          data: {
            ...
            refresh_token: __marketplace.refreshToken
          },
          headers: {
            "Content-type": config.MIME_FORM
          }
          ...
          function success(data) {
            if (data.status != 200) {
              ...
              var newRefreshToken = data.response.refresh_token;

              __marketplace.refreshToken = newRefreshToken;
              __marketplace.refreshFunction &&
                __marketplace.refreshFunction(newRefreshToken);
  ...
  mp.searchPayment = function(filters, offset, limit) {
    ...
    return mp.get(
      {
        uri: uriPrefix + "/v1/payments/search",
        params: filters
      },
      next
    );
  ...
  mp.getPayment = mp.getPaymentInfo = function(id) {
    ...
    return mp.get(
      {
        uri: uriPrefix + "/v1/payments/notifications/" + id
      },
      next
    );
  };
      ...
      if (error) {
        ...
        deferred.reject(
          new MercadoPagoError(
            body ? body.message || body : "Unknown",
            response.statusCode
          )
        );
      } else {
