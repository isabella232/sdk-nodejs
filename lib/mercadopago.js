	...
	*/
	mp.enableMarketplace = function (initRefreshToken, refreshFunction) {
		...
		if (typeof refreshFunction != "function") {
			throw new Error ("Must provide a callback (to get the new refresh_token each refresh!)");
		}
				...
				MPRestClient.post({
					...
					function success (data) {
						if (data.status != 200) {
							...
							__marketplace.refreshFunction && __marketplace.refreshFunction(newRefreshToken);
